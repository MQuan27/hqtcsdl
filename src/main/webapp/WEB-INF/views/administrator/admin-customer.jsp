		...
		<div id="page-wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h1>Khách Hàng</h1>
					<ol class="breadcrumb">
						<li><a href="${contextPage}/resources/back_end/admin/manage"><i
					...
					<div class="table-responsive">
							...
							<tbody>
								<c:if test="${not empty listStaff }">
									<c:forEach items="${listStaff }" var="list">
										<tr>
											<td>${list.fullname}</td>
											<td>${list.paymentMethod.description }</td>
											<td>${list.email}</td>
											<td>${list.datecreated}</td>
											<td><a
												href="${contextPage}/manage/deleteCustomer/?userId=${list.userId }"
								...
								<c:if test="${empty listStaff }">
									<tr>
										<td colspan="5"><h3>Danh sách khách hàng rỗng</h3></td>
									</tr>
								</c:if>
							</tbody>
						</table>
						<div>
							<a class="btn btn-success"
								href="${contextPage }/manage/addCustomerPage"> Thêm khách
								hàng </a>
						</div>
					</div>
				</div>
